...
Where:
- `k_c`, `k_t` are bin-level counts
- `T_c`, `T_t` are total counts across all tested bins
...
log2FC = log2( (k_c + ε) / (T_c + ε)  ÷  (k_t + ε) / (T_t + ε) )

````

Where:
- `ε` is a small pseudocount (default: 0.5)
...
Pooling replicates can allow a single replicate to dominate significance.
I-SAGE therefore supports an optional **replicate-aware meta-analysis**.
...
**Outputs & Interpretation** — how to read and interpret the results produced by the pipeline.
